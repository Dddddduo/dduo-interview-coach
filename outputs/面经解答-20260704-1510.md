...
> 生成日期: 2026-07-04 15:10
> 题目来源: 面经助手

---

# 题目：MySQL 事务过程中有哪些日志，各自作用
...
```
值=1: 事务提交 → redo buffer → OS buffer → fsync() ====磁盘==== (同步, 最安全)
值=0: 事务提交 → redo buffer ... (每秒刷一次) → OS buffer → fsync → 磁盘 (异步, 最快)
值=2: 事务提交 → redo buffer → OS buffer ... (每秒 fsync) → 磁盘 (折中)
```
...
**问题 1：Redo Log 太小导致频繁 Checkpoint**
```sql
-- 查看 Redo Log 使用率
SHOW ENGINE INNODB STATUS\G
-- Log sequence number - Last checkpoint at 的差值
-- 如果超过 Redo Log 总容量的 75%，需要增大容量
```
...
- 监控 LSN 差值：`SHOW ENGINE INNODB STATUS` 中 `Log sequence number - Last checkpoint at`

#### 思考 4：Binlog 三种格式的工程权衡
- **STATEMENT**：MySQL 5.1 前默认值，常导致主从不一致（如 LIMIT 排序不确定）
- **ROW**：5.7.7+ 默认值，但大事务产生巨量日志（批量 DELETE 100 万行记录 100 万条删除）
...
| **Undo Log** | 原子性 + MVCC + 版本链 + Redo 保护 Undo | 40s | 重点提 MVCC 和 Redo 关系 |
| **Redo Log** | 持久性 + WAL + 三级缓冲 + Crash Recovery | 60s | 展示底层原理理解 |
| **Binlog** | Server 层 + 3 种格式 + 主从复制 | 40s | 对比 Redo Log，突出区别 |
...
- **误区 2**："Binlog 也是 InnoDB 的日志" — 错误，Binlog 是 Server 层日志，MyISAM 也有
- **误区 3**："Redo Log 记录了未提交事务，所以崩溃恢复时会全部回滚" — 不准确，已提交的通过 Redo 恢复，未提交的通过 Undo 回滚
...
- **Redo → 两阶段提交**："Redo Log 属于 InnoDB 引擎层，而 Binlog 属于 Server 层——两个不同的日志系统如何保持一致性？这就引出了 MySQL 中非常核心的设计：两阶段提交。"
- **总结过渡**："当这三种日志协同工作，配合两阶段提交协议，MySQL InnoDB 就能同时保证事务的原子性、一致性和持久性。"

### 面试官可能的追问准备

1. "`innodb_flush_log_at_trx_commit` 设为 0，崩溃丢多少数据？" → 最多丢 1 秒
2. "Group Commit 是什么？" → 多个事务的 fsync 合并为一次
3. "MySQL 8.0 Undo 有什么变化？" → 独立表空间、自动截断
4. "如何监控 Redo Log 使用率？" → `SHOW ENGINE INNODB STATUS` 看 LSN 差异
