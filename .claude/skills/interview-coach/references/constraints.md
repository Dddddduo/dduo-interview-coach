...
> Resume 文件位于 `references/resume/resume.md`。Agent 在回答个人化问题时必须读取。

| # | 规则 |
|---|------|
| R1 | **回答个人化问题前，必须先读取 `references/resume/resume.md`**（P1 约束） |
| R2 | **所有个人化回答必须严格基于 resume.md 中的内容**，不可编造任何经历、项目、数据 |
| R3 | **量化数据必须与 resume.md 完全一致**：提效 70%、RT 700ms→50ms、关单 10→50 单/s、召回率提升 55%、可用率 99%、慢查询 3s+→200ms、粉丝 5000+、阅读量 2700 万+ 等 |
| R4 | **严禁**使用 resume.md 中不存在的项目名称、公司名称、技术栈、竞赛名次 |
| R5 | **resume.md 变更时**，Agent 自动读取最新版本；无需修改 Agent 定义 |
| R6 | 如果 resume.md 中没有相关信息来回答某个个人化问题，**如实说明信息不足**，基于已有内容给出最接近的回答，**严禁泛化编造** |
